...
channel_revenues AS (
    ...
    FROM ad_attributions a
    JOIN orders o ON a.customer_id = o.customer_id
    WHERE o.order_date BETWEEN '2023-10-01' AND '2023-11-30'  -- 包括后续30天转化
    AND a.attribution_date BETWEEN '2023-10-01' AND '2023-10-31'
...
SELECT 
    ...
    cr.total_revenue,
    cr.new_customers,
    ROUND(cr.total_revenue / NULLIF(cc.total_cost, 0), 2) AS roi,
    ROUND(cc.total_cost / NULLIF(cr.new_customers, 0), 2) AS cost_per_acquisition
FROM channels c
JOIN channel_costs cc ON c.channel_id = cc.channel_id
LEFT JOIN channel_revenues cr ON c.channel_id = cr.channel_id
ORDER BY roi DESC;
```
